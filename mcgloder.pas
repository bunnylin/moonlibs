...
{ Copyright 2005-2018 :: Kirinn Bunnylin / MoonCore                         }
{                                                                           }
...
// This assumes all 8-bit input values are in non-linear sRGB color space.
// sRGB has a perceptually somewhat uniform lightness gradient, whereas
// a linear color space has uniform light energy.
// Certain types of image processing (mostly color blending) must be done in
// linear RGB for best results, so here's a lookup transform from 8-bit sRGB
// to 16-bit linear values.
//
...
//   end;
//
// A hardcoded reverse table would take 64k bytes, so it's better to generate
// that at unit initialisation. The average pixel intensity error this
